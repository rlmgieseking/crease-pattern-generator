...
class DiagonalShift(Molecule):
    ...
    def generateverts(self):
        ...
        # Generate remaining points for the lower half of the shift
        self.verts, conv3dbot, conv2dbot = self.generatehalftwist(self.verts, conv3d, True)
        # Switch the rotational direction
        self.cwrot = not self.cwrot
        print(conv3dbot, conv2dbot)
        # Generate remaining points for the top half of the shift
        topedge, conv3dtop, conv2dtop = self.generatehalftwist(topedge, conv3d, False)
        print(conv3dtop, conv2dtop)
        
        # Add points from top half to self.verts
        # For middle joining points, average positions if they are not the same
        for i in range(self.ngores*2+1):
            ii = i + self.ngores*4+2
            topedge[ii].pos2d[0,1] += conv2dbot - conv2dtop
            print(self.verts[ii].pos2d, self.verts[ii].pos3d)
            print(topedge[ii].pos2d, topedge[ii].pos3d)
