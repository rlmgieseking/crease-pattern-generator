...
def glowscript(filename, shape, verts=False, edges=False, faces=True):
    out = open(filename, 'w')
    print('Glowscript')
    ...
    # Print coordinates of vertices and indices of edges and faces to generate faces later
    out.write('''Web VPython 3.2
...
verts = [''')
    for i in range(len(shape.verts)):
        v = shape.verts[i]
        if i > 0: out.write(' '*9)
        out.write('[' + str('%.2f'%v.pos3dvis[0,0]) + ', ' 
                  + str('%.2f'%v.pos3dvis[0,1]) + ', '
                  + str('%.2f'%v.pos3dvis[0,2]) + ']')
        if i != len(shape.verts) - 1:
            out.write(',\n')
        else:
    ...
    for i in range(len(shape.edges)):
        e = shape.edges[i]
        if i > 0: out.write(' '*9)
        out.write('[' + str(shape.verts.index(e.end1)) + ', ' + str(shape.verts.index(e.end2)) + ']')
    ...
    for i in range(len(shape.faces)):
        ...
        out.write('[')
        for v in f.verts:
            out.write(str(shape.verts.index(v)) + ',')
        if i != len(shape.faces) - 1:
            out.write('],\n')
        else:
            out.write(']]\n\n# Edge fold types\nedgetypes = [')
    for i in range(len(shape.edges)):
    ...
    for i in range(len(shape.faces)):
        f = shape.faces[i]
        color_scaling = np.dot(f.normal, lightvector) * color_norm_scaling
        color = f.color + color_scaling
        
        if i > 0: out.write(' '*9)
        out.write('[' + str(color[0,0]) + ', ' + str(color[0,1]) + ', ' + str(color[0,2]) + ']')
        if i != len(shape.faces) - 1:
            out.write(',\n')
        else:
            out.write(']\n\n')
    
    # Write the script to display the shape from these coordinates
    out.write('''caption = """Rotate: right-click or Ctrl+left-click and drag
Zoom: middle-click or Alt+left-click and drag
Pan: Shift+left-click and drag"""
scene = canvas(userzoom=True, userspin=True, 
               forward=vector(0,1,0), up=vector(0,0,1),
               center=vector(0,0,''' + str(shape.currentheight3d/2) + '''),
               background=color.white,
               caption=caption)
''')
    if edges:
        out.write('''for i in range(len(edges)): 
            edge = edges[i]
            dir = edgetypes[i]
...
''')
    if faces:
        out.write('''for i in range(len(faces)):
            face = faces[i]
            fcolor = facecolors[i]
            if len(face) == 3:
